...
def journalier_page():
    ...
    for data48h in response["hourly"]:
        ...
        if day != temp_day:
            url_holiday = (
                "https://holidays.abstractapi.com/v1/?api_key=%s&country=US&year=%s&month=%s&day=%s"
                % (api_key_holiday, year, month, day)
            )
    ...
    fig1 = px.histogram(
        ...
        y="prediction",
        color="date48",
        title="prédiction sur les prochaines 48h",
        barmode="group",
    )
